...
Demo 已固定到 `6.3.2` tag 的 `YKIFLYADLib.podspec`；公开可用性以同版本 GitHub Release 和发布后 CI 为准。
...
`6.3.2` 默认仍要求点击视图位于广告容器内。优酷媒体确需容器外 CTA 时显式开启 `allowsExternalClickViews` 并处理 `nativeFeedAd:didRejectClickWithError:`。外部点击视图可在绑定后才挂载和布局，不要求与 containerView 存在共同层级、包装器或距离关系，也不限制其占 window 的面积比例；containerView 的非页面根父级/祖先可以作为点击层。点击时仍须通过同 window/scene、CTA 可见可交互、广告容器前台可见比例至少 2/3，以及当前 Ad/container/generation 的独占租约校验。父级注册 CTA 会兜底普通 `UIView` 子视图；触摸路径上的媒体 `UIControl` 或媒体自有手势优先，显式注册的 CTA 自身仍由 SDK 处理，`closeView` 子树继续隔离。失败以带稳定 point 和中文处理提示的 71503 拒绝，不曝光、不监测、不跳转。固定单容器页面可按需使用 `detachFromCurrentContainer`。
