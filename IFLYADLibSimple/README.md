...
Demo 已固定到 `6.3.1` tag 的 `YKIFLYADLib.podspec`；公开可用性以同版本 GitHub Release 和发布后 CI 为准。
...
`6.3.1` 默认仍要求点击视图位于广告容器内。优酷媒体确需容器外 CTA 时显式开启 `allowsExternalClickViews` 并处理 `nativeFeedAd:didRejectClickWithError:`。外部点击视图可在绑定后才挂载和布局，不要求与 containerView 存在共同层级、包装器或距离关系，也不再限制其占 window 的面积比例；containerView 的非页面根父级/祖先可以作为点击层。点击时仍须通过同 window/scene、CTA 可见可交互、广告容器前台可见比例至少 2/3，以及当前 Ad/container/generation 的独占租约校验。未提供 `closeView` 时，父 `clickView` 只处理最终命中自身的触摸，任意子视图点击不进入广告点击链路；失败继续以带稳定 point 的 71503 拒绝，不曝光、不监测、不跳转。固定单容器页面可按需使用 `detachFromCurrentContainer`。
...
Demo 使用以下八个与私有库 Demo 一致的联调广告位，集中配置在
`Supporting Files/IFLYAdPrefixHeader.pch`：

| 场景 | 配置宏 | 广告位 ID |
| --- | --- | --- |
| 开屏图片 | `__SPLASH_NATIVE_AD_UNIT_ID__` | `D65CC066BF3AE066C48F4A101A30005C` |
| 开屏视频 | `__SPLASH_VIDEO_AD_UNIT_ID__` | `DDC620929747D6DB85FFEB14AB893F4E` |
| 插屏竖图 | `__INTERSTITIAL_AD_UNIT_ID__` | `22E131CFF097584531EAB3281C40769C` |
| 插屏横图 | `__INTERSTITIAL_LANDSCAPE_IMAGE_AD_UNIT_ID__` | `28AA4CE6390C2F6A874B9425C2223E0A` |
| 插屏竖视频 | `__INTERSTITIAL_PORTRAIT_VIDEO_AD_UNIT_ID__` | `2C87A382F3D0CFFC844328E8C8CD8B51` |
| 插屏横视频 | `__INTERSTITIAL_LANDSCAPE_VIDEO_AD_UNIT_ID__` | `284707795BA1F6A692D315CD02CDD78D` |
| 自渲染信息流图片 | `__TYPED_ONE_NATIVE_AD_UNIT_ID__` | `24556006C29E3C51A624F4B704BC7345` |
| 自渲染信息流视频 | `__FEED_VIDEO_AD_UNIT_ID__` | `3AE3AC7B093A4CC6A1E2F19CF16709FC` |

插屏横竖版选择会使用各自对应的图片或视频广告位。
